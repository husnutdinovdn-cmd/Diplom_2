"""Тесты API: создание заказа (POST /api/orders)."""
import requests
import pytest
import allure
...
class TestCreateOrder:
    """Заказ: с авторизацией, без авторизации, с/без ингредиентов, неверный хеш."""
    ...
    def test_create_order_without_auth(self, base_url, valid_ingredient_ids):
        ...
        # По документации ожидается 401; некоторые версии API принимают заказ и возвращают 200
        with allure.step("Без токена: допустим 401 (по док.) или 200 (если API разрешает)"):
    ...
    def test_create_order_without_ingredients(self, base_url, auth_headers):
        ...
        with allure.step("Ожидается 400 Bad Request"):
    ...
    def test_create_order_invalid_ingredient_hash(self, base_url, auth_headers):
        ...
        with allure.step("Ожидается 500 Internal Server Error"):
